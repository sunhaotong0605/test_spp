...
class MultiHeadAttention(nn.Module):
    def __init__(self, config, omics_of_interest_size: int, other_omic_size: int, position_embedding_type=None):
        super().__init__()
        # 检查配置中隐藏层的大小是否能被注意力头的数量整除，如果不能，且配置中没有定义 embedding_size，则抛出一个错误
        if config.hidden_size % config.num_attention_heads != 0 and not hasattr(
            config, "embedding_size"
        ):
            raise ValueError(
        ...
        #self.dropout = nn.Dropout(config.attention_probs_dropout_prob)
        # 设置位置嵌入的类型，如果提供了 position_embedding_type 参数则使用它，否则从配置中获取，如果没有则默认为 "absolute"
        # 此处代码有问题，NT的位置编码属性是"nt_position_embedding_type": "rotary"
        self.position_embedding_type = position_embedding_type or getattr(
            config, "position_embedding_type", "absolute"
        )
        # 初始化一个变量来存储旋转位置嵌入的实例，如果使用则非 None
        self.rotary_embeddings = None
        # 如果位置嵌入类型是 "relative_key" 或 "relative_key_query"，则初始化最大位置嵌入数和距离嵌入层
        if (
            self.position_embedding_type == "relative_key"
            or self.position_embedding_type == "relative_key_query"
        ):
